...
library(drc)
library(dr4pl)
source("~/Documents/GitHub/sushi/scripts/utils/kitchen_utensils.R")

data_dir <- "~/Downloads/ADC_manuscript_data/"

# -----
# Load the raw data ----
# ---- 

compound_annotations <- data.table::fread(file.path(data_dir, "Ab_ADCs for manuscript.csv"))

filtered_counts <- data.table::fread(file.path(data_dir, "Ab_ADC_filtered_counts_sequencing.csv")) 

# ----
# Normalize relative to the spike-in control barcodes ----
# ----

source("~/Documents/GitHub/sushi/scripts/normalize/normalize_functions.R")
...
source("~/Documents/GitHub/sushi/scripts/well_metrics/well_metrics_functions.R")

source("~/Documents/GitHub/sushi/scripts/qc_tables/qc_tables_functions.R")
...
source("~/Documents/GitHub/sushi/scripts/compute_l2fc/compute_l2fc_functions.R")
source("~/Documents/GitHub/sushi/scripts/bias_correction/bias_correction_functions.R")
source("~/Documents/GitHub/sushi/scripts/collapse_replicates/collapse_replicates_functions.R")
...
l2fc = compute_l2fc(normalized_counts = normalized_counts,
  ...
  # dplyr::anti_join(filtered_counts %>%         ## antijoin only to remove poscons?          
  #                    dplyr::distinct(CompoundPlate, SampleID, depmap_id, cell_set, lua, pool_id)) %>% 
...
source("~/Documents/GitHub/sushi/scripts/drc/dose_response_functions.R")
...
plates <- plates[3:6]
# why plates[3:6]? Is this for testing purposes?
...
qc_table %>% 
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqQC.csv"))

l2fc %>% 
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqLFC.csv"))

collapsed_l2fc %>% 
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqLFCCollapsed.csv"))

drc_table %>% 
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqDRC.csv"))


# Portal files --- !

compound_annotations <- data.table::fread(file.path(data_dir, "PRISMOncologyReferenceCompoundList.csv"))

filtered_counts <- data.table::fread(file.path(data_dir, "PRISMOncologyReferenceSeqFilteredCounts.csv"))

qc_table <- data.table::fread(file.path(data_dir, "PRISMOncologyReferenceSeqQC.csv"))

drc_table <- data.table::fread(file.path(data_dir, "PRISMOncologyReferenceSeqDRC.csv"))

collapsed_l2fc <- data.table::fread(file.path(data_dir, "PRISMOncologyReferenceSeqLFCCollapsed.csv"))

l2fc <- data.table::fread(file.path(data_dir, "PRISMOncologyReferenceSeqLFC.csv"))
...
drc_table %>% 
  ...
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqResponseCurves.csv"))
...
drc_table %>% 
  ...
  write.csv(file.path(data_dir, "PRISMOncologyReferenceSeqLog2AUCMatrix.csv"))
...
collapsed_l2fc %>%
  ...
  write.csv(file.path(data_dir, "PRISMOncologyReferenceSeqLog2ViabilityCollapsedMatrix.csv"))
...
collapsed_l2fc %>%
  ...
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqLog2ViabilityCollapsedConditions.csv"))
...
LFC_ %>% 
  ...
  write.csv(file.path(data_dir, "PRISMOncologyReferenceSeqViabilityMatrix.csv"))

LFC.conditions %>% 
  write_csv(file.path(data_dir, "PRISMOncologyReferenceSeqViabilityConditions.csv"))
...
conf.pools %>% 
  ...
  write.csv(file.path(data_dir, "PRISMOncologyReferenceSeqConfounderMatrix.csv"))
