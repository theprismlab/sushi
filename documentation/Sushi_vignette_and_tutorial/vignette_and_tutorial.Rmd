...
local_sushi_path = "~/GitHub/theprismlab/sushi"
...
# Pull column schema from the github repo
schema_path = file.path(local_sushi_path, "sushilib", "sushi_io", "schema.yml")
```
...
**Sushi** is the PRISM pipeline designed to process raw barcode counts from sequencing experiments. The sushi pipeline github repository can be found at <https://github.com/theprismlab/sushi/tree/main>
...
# Set up inputs ----
filtered_counts <- read_data_table(file.path(local_test_data_path, "filtered_counts.csv"), schema_path)
CB_meta <- read_data_table(file.path(local_test_data_path, "CB_meta.csv"), schema_path)
pseudocount <- 0
input_id_cols <- c("pcr_plate", "pcr_well")
negcon_cols <- c("pcr_plate", "pert_vehicle")

# Identify control barcodes that can be used for normalization
...
normalized_counts %>% 
  ...
  mutate(pcr_profile = paste0(pcr_plate,";", pcr_well),
         cb_log2_dose = round(cb_log2_dose, 5)) %>%
  ...
  labs(x = 'log2(n)', y = 'log2(dose)', color = "CB log2 dose") + 
...
### Measures of pool quality {.tabset}

In the control wells, we identify and drop any outlier pools. Outlier pools are defined as pools where the absolute median deviation of its cell lines from the median log2 normalized value is greater than 2.

$$
|median(log_2norm\ n - median(log_2norm\ n))| > 2
$$

Control PCR wells where more than 25% of the pools are identified as outliers are also filtered out. Pools that that identified as outliers in more than 25% of the control PCR wells of a PCR plate are also dropped.
...
# Set up inputs ----
normalized_counts <- read_data_table(file.path(local_test_data_path, "normalized_counts.csv"), schema_path)
...
cell_line_cols <- c("cell_set", "lua", "depmap_id", "pool_id", "growth_condition")
...
qc_data <- read_data_table(file.path(local_test_data_path, "qc_table.csv"), schema_path)
...
To apply the bias correction, there must be a column in the data frame that describes the growth pattern of a cell line or pool and a column containing log2 normalized counts of the negative controls.
...
growth_pattern_col = "growth_condition"
...
# Set up inputs ----
l2fc <- read_data_table(file.path(local_test_data_path, "l2fc.csv"), schema_path)
...
After collapsing across biological replicates, a monotonicity filter is applied only for MTS style screens. This filter identifies and removes pools that break our assumptions of monotonicity.

```{r example monotonicity, echo = T, eval = T}
# Set up inputs ----
...
# Identify pools that break monotonicity ----
monotonicity = get_monotonicity(collapsed_l2fc, trt_cl_cols)
flagged_trt_pools = flag_breaks(monotonicity, 
                                trt_pool_cols = trt_pool_cols, 
                                trt_cell_set_cols = sig_cols, 
                                pool_cutoff = 0.25)

head(flagged_trt_pools |> dplyr::filter(!is.na(cell_set_flag)))
```
...
drc_table = read_data_table(file.path(local_test_data_path, "drc_table.csv"), schema_path)
...
# Set up inputs ----
depmap_file_path <- file.path(local_test_data_path, "prism_biomarker_public_0625.h5")
collapsed_l2fc <- read_data_table(file.path(local_test_data_path, "collapsed_l2fc.csv"), schema_path)
cell_line_cols <- c("depmap_id","lua","pool_id")
sig_cols <- c("cell_set","pert_name","pert_dose","pert_dose_unit","day")
