...
deliberate: the alternative is writing Jenkins API tokens to disk. It also
...
def verify(user: str, token: str) -> dict:
    ...
    if not user.strip() or not token.strip():
        raise AuthError("Both a Jenkins user and an API token are required.")
    try:
        r = requests.get(f"{jenkins.BASE}/me/api/json", auth=(user, token),
                         timeout=jenkins.TIMEOUT)
    except requests.RequestException as exc:
        raise AuthError(f"Could not reach Jenkins at {jenkins.BASE}: {exc}") from exc

    if r.status_code in (401, 403):
        raise AuthError("Jenkins rejected those credentials. Use your Jenkins user ID and an "
                        "API token from /user/<you>/configure -- not your web password.")
    if not r.ok:
...
def create(user: str, token: str) -> tuple[str, dict]:
    """Verify, then remember the credentials against an opaque session id."""
    identity = verify(user, token)
    _evict_expired()
    sid = secrets.token_urlsafe(32)
    _sessions[sid] = {**identity, "user": user, "token": token, "created": time.time()}
    return sid, identity
...
def auth_for(sid: str | None) -> tuple[str, str] | None:
    ...
    entry = get(sid)
    return (entry["user"], entry["token"]) if entry else None
